...
def save_process_video(duration=int, input_video: str = INPUT_VIDEO, output_video: str = PROCESS_VIDEO):
    ...
    audio_clipped = AudioFileClip(RESOURCES_FOLDER+tmp_audio_path).subclipped(start_time=0, end_time=duration)
    audio_clipped.write_audiofile(RESOURCES_FOLDER+tmp_audio_path_clipped, codec="mp3")
    video_with_analysis.write_videofile(RESOURCES_FOLDER+process_video_with_audio, codec="libx264",
